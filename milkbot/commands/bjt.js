...
function resolveTournament() {
  ...
  for (const p of players) {
    ...
    if (p.busted || (pt > 21 && !playerBJ)) {
      ...
      balances[p.userId] = Math.min(1_000_000_000, (balances[p.userId] || 0) + buyIn);
      pushers.push(p);
    } else if (playerBJ) {
      ...
      balances[p.userId] = Math.min(1_000_000_000, (balances[p.userId] || 0) + buyIn + bjProfit);
      winners.push(p);
      ...
      balances[p.userId] = Math.min(1_000_000_000, (balances[p.userId] || 0) + buyIn + profit);
      winners.push(p);
